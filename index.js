...
//        o888o   o888o o888ooooood8 o888ooooood8 o888o         `8.     88     .8'   
            ...
            case 'help':
                    ...
                    const embed = new Discord.MessageEmbed()
                        ...
                        .setImage("https://cdn.discordapp.com/attachments/987820203016618015/1099730002355957850/LISTE_DES_COMMANDES_bleu.gif")
                        ...
                        **autrespays**
                        Si aucun des rôles de <@&987841964542132294> disponibles dans <#987820202752356375> vous conviennent, si vous n'êtes pas de France, vous pouvez taper cette commande pour faire apparaître un menu pour choisir d'autres pays.
                        `)
...
client.on('messageCreate', async message => {
  ...
  if (command === 'quirole&') {
    ...
    const membersList = membersWithBothRoles.map(member => member.user.tag).join(', ');
    const embed = new MessageEmbed()
      .setColor('GREEN')
      .setTitle(`${roles[0].name} & ${roles[1].name}`)
      .setDescription(`Les membres ayant les deux rôles :\n${membersList}`);
...
client.on('messageCreate', async message => {
  if (message.content === '!deleteChannelsap') {
    const channels = [
      "『🇩🇿』𝐴𝑙𝑔é𝑟𝑖𝑒",
      "『🇩🇪』𝐴𝑙𝑙𝑒𝑚𝑎𝑔𝑛𝑒",
      "『🇦🇩』𝐴𝑛𝑑𝑜𝑟𝑟𝑒",
    ...
    for (const channelName of channels) {
      const channel = message.guild.channels.cache.find(c => c.name === channelName && c.type === 'GUILD_TEXT');
      if (channel) {
        await channel.delete();
      }
    }
...
client.on('messageCreate', async message => {
  if (message.content === '!createChannelsap') {
    const categoryId = '1099083049372750025';
    const roleIds = [
      "1098507604759687178",
      "1098502027216179210",
      "1098505293178093569",
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId === 'liste_des_commandes') {
    const embed = new MessageEmbed()
      .setColor("YELLOW")
      .setFooter({ text: "Au nom de l'équipe 𝐺𝑂𝑀𝑈𝑆𝐶𝑈." })
      .setImage("https://cdn.discordapp.com/attachments/987820203016618015/1099729685253984376/LISTE_DES_COMMANDES_jaune.gif")
      .setThumbnail("https://cdn.discordapp.com/attachments/987820203016618015/1088231600854143077/gars_et_fille_body.png")
      .setDescription(`
